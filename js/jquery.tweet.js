(function($) {
  ...
  $.fn.tweet = function(o){
    ...
    return this.each(function(i, widget){
      ...
      list.delegate("time", "updatetime", function() {
          var tweet_timeStr = this.getAttribute("datetime");
          var tweet_time = new Date(tweet_timeStr).getTime();
          var tweet_relative = relative_time(tweet_time);
          $(this).find("a").text(tweet_relative);
      });
      ...
      $(widget).bind("load", function(){
        $.getJSON(build_url(), function(data){
          ...
          var tweets = $.map(data.results || data, function(item){
            ...
            var tweet_time_unit = tweet_relative_time.replace(/^.* (\w+) ago/, "$1");
